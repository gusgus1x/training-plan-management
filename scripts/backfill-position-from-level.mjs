...
// Matches source rows to employee via (company_id, EmpID) since employee_code now holds EmpID.
...
let sourceRows;
try {
  sourceRows = (await sourcePool.request().query("SELECT EmpID, ComCode, PositionLevel FROM dbo.View_EmpInfoForTrainee")).recordset;
...
const summary = { updated: 0, skippedBlankEmpId: 0, skippedAlreadySet: 0, skippedNoPositionLevel: 0, skippedNoMatchingPosition: 0, skippedNoMatchingEmployee: 0, skippedUnknownCompany: 0 };
...
try {
  ...
  const employees = (await new sql.Request(transaction).query("SELECT employee_id, company_id, employee_code, position_id FROM dbo.employee")).recordset;
  const employeeByKey = new Map(employees.map((e) => [`${e.company_id}|${e.employee_code}`, e]));
  ...
  for (const row of sourceRows) {
    ...
    const employee = employeeByKey.get(`${companyId}|${empId}`);
  ...
  console.log(`Done. Updated: ${summary.updated}, Skipped (blank EmpID): ${summary.skippedBlankEmpId}, Skipped (already set): ${summary.skippedAlreadySet}, Skipped (blank PositionLevel): ${summary.skippedNoPositionLevel}, Skipped (no matching position): ${summary.skippedNoMatchingPosition}, Skipped (no matching employee): ${summary.skippedNoMatchingEmployee}, Skipped (unknown company): ${summary.skippedUnknownCompany}.`);
