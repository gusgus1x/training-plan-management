/*
TrainingPlanManagement — Instructor API least-privilege permissions.
Role enforcement remains in the API; CRUD is limited to dbo.instructor.
*/
...
IF OBJECT_ID(N'dbo.instructor', N'U') IS NULL
    THROW 52703, 'Required table dbo.instructor was not found.', 1;
IF OBJECT_ID(N'dbo.training_plan_oap', N'U') IS NULL
    THROW 52704, 'Required table dbo.training_plan_oap was not found.', 1;
GO

GRANT SELECT, INSERT, UPDATE, DELETE
ON OBJECT::dbo.instructor
TO [training_plan_app];
GO

-- Required only to determine whether delete must become INACTIVE.
GRANT SELECT
...
WHERE grantee_principal_id = DATABASE_PRINCIPAL_ID(N'training_plan_app')
  AND major_id IN
      (OBJECT_ID(N'dbo.instructor'), OBJECT_ID(N'dbo.training_plan_oap'))
