...
const mergeDefaultPositions = (storedRows: PositionRecord[]): PositionRecord[] => {
  const existingMap = new Map(
    storedRows.map((r) => [r.positionCode.trim().toLowerCase(), r]),
  );
  return defaultPositionRows.map((defaultRow) => {
    const existing = existingMap.get(defaultRow.positionCode.toLowerCase());
    return existing
      ? {
          ...defaultRow,
          ...existing,
          positionNameTh: defaultRow.positionNameTh,
          positionNameEn: defaultRow.positionNameEn,
        }
      : defaultRow;
  });
};
