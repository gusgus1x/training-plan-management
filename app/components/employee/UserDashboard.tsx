                ...
                <>
                  ...
                  {isTrainingDetailsOpen ? (
                    <div className={styles.trainingDetailsCard} role="region" aria-label="Training details">
                      <div className={styles.trainingDetailsHeader}>
                        <div className={styles.trainingDetailsTitle}>
                          <span>📋</span>
                          <strong>{isThai ? "รายละเอียดหลักสูตรที่ต้องเข้าอบรม" : "Upcoming Training Details"}</strong>
                        </div>
                        <button
                          type="button"
                          className={styles.trainingDetailsCloseBtn}
                          onClick={() => setIsTrainingDetailsOpen(false)}
                          aria-label="Close"
                        >
                          ✕
                        </button>
                      ...
                      <div className={styles.trainingDetailsGrid}>
                        <div className={styles.trainingDetailItem}>
                          <span className={styles.trainingDetailLabel}>{isThai ? "รหัสหลักสูตร" : "Course Code"}</span>
                          <strong className={styles.trainingDetailValue}>{nextTraining.plan.courseCode || "-"}</strong>
                        </div>
                        <div className={styles.trainingDetailItem}>
                          <span className={styles.trainingDetailLabel}>{isThai ? "ชื่อหลักสูตร" : "Course Name"}</span>
                          <strong className={styles.trainingDetailValue}>{nextTraining.plan.courseName}</strong>
                        </div>
                        <div className={styles.trainingDetailItem}>
                          <span className={styles.trainingDetailLabel}>{isThai ? "วันและเวลาอบรม" : "Schedule"}</span>
                          <strong className={styles.trainingDetailValue}>
                            {new Date(nextTraining.plan.startAt).toLocaleDateString(locale, {
                              year: "numeric",
                              month: "short",
                              day: "numeric",
                            })} ({new Date(nextTraining.plan.startAt).toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" })} - {new Date(nextTraining.plan.endAt).toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" })})
                          </strong>
                        </div>
                        <div className={styles.trainingDetailItem}>
                          <span className={styles.trainingDetailLabel}>{isThai ? "สถานที่ / ห้องอบรม" : "Venue / Room"}</span>
                          <strong className={styles.trainingDetailValue}>{nextTraining.plan.venue || "-"}</strong>
                        </div>
                        <div className={styles.trainingDetailItem}>
                          <span className={styles.trainingDetailLabel}>{isThai ? "วิทยากร" : "Instructor"}</span>
                          <strong className={styles.trainingDetailValue}>{nextTraining.plan.instructor || "-"}</strong>
                        </div>
                        <div className={styles.trainingDetailItem}>
                          <span className={styles.trainingDetailLabel}>{isThai ? "ระยะเวลา" : "Duration"}</span>
                          <strong className={styles.trainingDetailValue}>{nextTraining.plan.hours} {isThai ? "ชั่วโมง" : "hrs"}</strong>
                        </div>
                        <div className={styles.trainingDetailItem}>
                          <span className={styles.trainingDetailLabel}>{isThai ? "สถานะการลงทะเบียน" : "Status"}</span>
                          <strong className={styles.trainingDetailValue} style={{ color: "var(--ui-30-primary-strong)" }}>
                            ✓ {nextTraining.status}
                          </strong>
                        </div>
                        <div className={styles.trainingDetailItem}>
                          <span className={styles.trainingDetailLabel}>{isThai ? "ผู้จัดอบรม" : "Organizer"}</span>
                          <strong className={styles.trainingDetailValue}>
                            {nextTraining.plan.owner === "CENTER" ? "HRD Center" : `${employeeCompany || "Factory"}`}
                          </strong>
                        </div>
                      </div>

                      {upcomingApprovedTrainings.length > 1 ? (
                        <div style={{ marginTop: 6 }}>
                          <button
                            type="button"
                            className={styles.otherTrainingsToggleBtn}
                            onClick={() => setShowAllUpcomingTrainings((prev) => !prev)}
                            aria-expanded={showAllUpcomingTrainings}
                            title={isThai ? "เปิด/ปิดดูหลักสูตรอื่นๆ ที่ต้องเข้าอบรม" : "Toggle other scheduled courses"}
                          >
                            <span>
                              📚 {isThai ? `หลักสูตรอื่นๆ ที่ต้องเข้าอบรม (${upcomingApprovedTrainings.length - 1} รายการ)` : `Other Scheduled Courses (${upcomingApprovedTrainings.length - 1})`}
                            </span>
                            <span style={{ fontSize: "0.75rem", color: "var(--ui-30-primary-strong)" }}>
                              {showAllUpcomingTrainings ? (isThai ? "▲ ซ่อนรายการ" : "▲ Hide") : (isThai ? "▼ คลิกเพื่อดู" : "▼ Show")}
                            </span>
                          </button>

                          {showAllUpcomingTrainings ? (
                            <div className={styles.otherTrainingsList}>
                              {upcomingApprovedTrainings.slice(1).map((item) => (
                                <div className={styles.otherTrainingRow} key={item.id}>
                                  <span className={styles.otherTrainingDate}>
                                    📅 {new Date(item.plan.startAt).toLocaleDateString(locale, { day: "numeric", month: "short" })}
                                  </span>
                                  <div className={styles.otherTrainingInfo}>
                                    <strong className={styles.otherTrainingName} title={item.plan.courseName}>
                                      {item.plan.courseName}
                                    </strong>
                                    <span className={styles.otherTrainingMeta}>
                                      {item.plan.venue || "-"} • {item.plan.hours} hrs • {item.status}
                                    </span>
                                  </div>
                                </div>
                              ))}
                      ...
                      <div className={styles.trainingDetailsActions}>
                        <button
                          type="button"
                          className={styles.detailsActionBtn}
                          onClick={() => setActiveModule("calendar")}
                        >
                          📅 {isThai ? "ดูในปฏิทินอบรม" : "View in Calendar"}
                        </button>
                        <button
                          type="button"
                          className={styles.detailsActionBtn}
                          onClick={() => setActiveModule("register")}
                        >
                          📋 {isThai ? "ดูการลงทะเบียนทั้งหมด" : "View All Registrations"}
                        </button>
                      </div>
                    </div>
                  ) : null}
