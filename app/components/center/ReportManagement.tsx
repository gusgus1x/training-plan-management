...
const extraReportItems = [
  {
    ...
    description: "ส่งข้อมูลให้พนักงานหรือ HRD Factory พร้อมติดตามข้อมูลที่ส่งกลับมา",
  },
] as const;
...
const centerMessageCompanies = ["ATA", "ATFB", "NIC", "SATI", "SNF", "TEP"] as const;
const centerMessageDepartments = ["HRD", "Production", "Operations", "Quality", "Maintenance", "Safety"] as const;
...
export default function ReportManagement({
  ...
  const [centerMessageRecipient, setCenterMessageRecipient] = useState("Employee");
  const [centerMessageCompany, setCenterMessageCompany] = useState<(typeof centerMessageCompanies)[number]>("ATA");
  const [centerMessageDepartment, setCenterMessageDepartment] =
    useState<(typeof centerMessageDepartments)[number]>("HRD");
  const [centerMessageSubject, setCenterMessageSubject] = useState("");
  const [centerMessageBody, setCenterMessageBody] = useState("");
  const [sentCenterMessages, setSentCenterMessages] = useState<SentCenterMessage[]>([]);
  const shouldShowCenterMessageCompany = centerMessageRecipient !== "HRD Center";
  const shouldShowCenterMessageDepartment = centerMessageRecipient === "Employee";
  ...
  const handleSendCenterMessage = () => {
    if (!centerMessageSubject.trim() || !centerMessageBody.trim()) {
      return;
    }
    ...
    setSentCenterMessages((current) => [
      {
        id: `CENTER-MESSAGE-${Date.now()}`,
        sender: "HRD Center",
        recipient: centerMessageRecipient,
        company: shouldShowCenterMessageCompany ? centerMessageCompany : "",
        department: shouldShowCenterMessageDepartment ? centerMessageDepartment : "",
        subject: centerMessageSubject,
        message: centerMessageBody,
        sentAt,
      },
      ...current,
    ]);
    setCenterMessageSubject("");
    setCenterMessageBody("");
  };
        ...
        <section className={styles.reportWorkspace} aria-label="HRD Factory export">
          <section className={styles.reportToolbar}>
            <div>
              ...
              <span>{sentCenterMessages.length} messages</span>
            </div>
          </section>

          <section className={styles.factoryPanel}>
            <section className={styles.factoryFormPanel} aria-label="Send message">
              ...
              <form className={styles.messageFormGrid}>
                <label>
                  <span>ส่งให้ใคร</span>
                  <select
                    value={centerMessageRecipient}
                    onChange={(event) => setCenterMessageRecipient(event.target.value)}
                  >
                ...
                {shouldShowCenterMessageCompany ? (
                  <label>
                    <span>บริษัท</span>
                    <select
                      value={centerMessageCompany}
                      onChange={(event) =>
                        setCenterMessageCompany(event.target.value as (typeof centerMessageCompanies)[number])
                      }
                    >
                      {centerMessageCompanies.map((company) => (
                ...
                {shouldShowCenterMessageDepartment ? (
                  <label>
                    <span>แผนก</span>
                    <select
                      value={centerMessageDepartment}
                      onChange={(event) =>
                        setCenterMessageDepartment(event.target.value as (typeof centerMessageDepartments)[number])
                      }
                    >
                      {centerMessageDepartments.map((department) => (
                ...
                <label>
                  ...
                  <input
                    type="text"
                    value={centerMessageSubject}
                    onChange={(event) => setCenterMessageSubject(event.target.value)}
                ...
                <label>
                  <span>ข้อความ</span>
                  <textarea
                    value={centerMessageBody}
                    onChange={(event) => setCenterMessageBody(event.target.value)}
                ...
                <button
                  ...
                  type="button"
                  disabled={!centerMessageSubject.trim() || !centerMessageBody.trim()}
            ...
            <section className={styles.factoryReplyPanel} aria-label="Received messages">
              <div className={styles.factoryPanelHeader}>
                <div>
              ...
              <div className={styles.factoryReplyList}>
                {sentCenterMessages.length > 0 ? (
                  sentCenterMessages.map((message) => (
