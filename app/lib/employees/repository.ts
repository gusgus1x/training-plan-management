...
const map=(r:any):EmployeeRecord=>({employeeId:String(r.employee_id),companyId:String(r.company_id),companyCode:r.company.company_code,employeeCode:r.employee_code,
  ...
  levelId:r.level_id?String(r.level_id):null,levelCode:r.employee_level?.level_code??null,levelKey:r.employee_level?.level_key??null,
...
const include={company:true,organization_function:true,position:true,employee_level:true} as const;
const data=(i:EmployeeInput,p:{hash:string;encrypted:Buffer;last4:string;keyVersion:number})=>({company_id:BigInt(i.companyId),function_id:i.functionId?BigInt(i.functionId):null,position_id:i.positionId?BigInt(i.positionId):null,level_id:i.levelId?BigInt(i.levelId):null,
...
export const employeeRepository={
  ...
  async referencesExist(i:EmployeeInput){const db=getPrismaClient();const [company,fn,pos,lvl]=await Promise.all([db.company.findUnique({where:{company_id:BigInt(i.companyId)}}),i.functionId?db.organization_function.findUnique({where:{function_id:BigInt(i.functionId)}}):true,i.positionId?db.position.findUnique({where:{position_id:BigInt(i.positionId)}}):true,i.levelId?db.employee_level.findUnique({where:{level_id:BigInt(i.levelId)}}):true]);return Boolean(company&&fn&&pos&&lvl)},
