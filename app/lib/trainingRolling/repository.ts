...
const mapRollingPlan = (row: RollingPlanWithRelations) => {
  ...
  const hasResults =
    Array.isArray(row.training_enrollment) && row.training_enrollment.length > 0;
  ...
  return {
    ...
    oapBudgetInstructor: oap?.budget_instructor?.toString() || "0",
    oapBudgetTraveling: oap?.budget_traveling?.toString() || "0",
    oapBudgetSeminarRoom: oap?.budget_seminar_room?.toString() || "0",
